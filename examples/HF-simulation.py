...
# * T_2 = 2 s
# * T_2^* = 1 ms
# Example: (HF science run) liquid Xe-129 at 14 T excited by N pulses with delays.
#
import time
...
T1 = 1424 * unit.s  # very long longitudinal relaxation (not the focus here)
# T2 = 10.0 * unit.min  # transverse relaxation time
T2 = 2.0 * unit.s  # transverse relaxation time
...
sample = Sample(
    ...
    T2=T2,
    T1=T1,
    vol=1 * unit.cm**3 * 0.26,  # considering the natural abundance
    pol=1 * unit.percent,
    mu=mu_Xe129,
    temp=163 * unit.K,
    verbose=False,
...
tip_angles = (1 / 180) * PI * np.ones(Npulses)

# delays = np.arange(55, 55 * (Npulses + 1), 5) * unit.ms
delays = np.ones(Npulses) * 5 * unit.s
...
pulseDur = 10 / simuRate
duration = np.sum(delays) + Npulses * pulseDur + delays[0]
# duration = T1 * 2
check(duration.si)
# ──────────────────────────────────────────────────────────────────────────── #
# Magnet
...
print(f"Larmor freq  = {nu_L.to(unit.Hz):g}")
print(f"T2*          = {simu.T2star.si:g}")
print(f"Tdelta       = {simu.Tdelta.si:g}")
print(f"numSteps     = {simu.numSteps}")
print(f"timeStep     = {simu.timeStep.si:g}")
...
simu.generateTrajectories(verbose=True)
toc = time.perf_counter()
print(f"generateTrajectories time = {toc - tic:.3f} s")

simu.keepMeanStd()
simu.displayTrjry(verbose=True)
# print("Remaining polarization =", simu.M_mean[0, ] simu.M0eqb)
