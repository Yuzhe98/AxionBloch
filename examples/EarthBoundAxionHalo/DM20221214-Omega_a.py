...
t0 = Time(datetime(2022, 12, 14, 13, 1, tzinfo=ZoneInfo("Europe/Berlin")))
t_hours = np.linspace(0, 8, 8) * unit.hour
meas_starts = t0 + t_hours  # every 1 hour
...
# ax_r.plot(t_hours, result["Omega_a_r"], color="tab:blue")
# # ax_r.plot(t_hours, result["grad_r"].imag, label="Im", color="tab:orange", ls="--")
# ax_theta.plot(t_hours, result["Omega_a_theta"], color="tab:green")
