...
def expDecayWindow(num: int, NC: float = -0.003, verbose=False):
    """
    ...
    if NC <= 0:
        raise ValueError(
            f"[{expDecayWindow.__name__}] decay number index constant <= 0"
        )
    indices = np.arange(num, dtype=float)
    window = np.exp((-1) * indices / NC)  # (1 - np.exp(decayFactor / num)) *
    if verbose:
        plt.figure()
        plt.scatter(indices, window)
        plt.plot(indices, window, label=f"exp({NC:.1f} i / {num:d})")
        plt.xlabel("Number")
        plt.ylabel("Window")
        plt.title(
...
def LIAFilterHomegaSquared1(
    ...
    return a / (
        (1.0 + (2.0 * np.pi * (frequency - demodFreq) * taun) ** 2.0) ** (8.0 / 1.0)
    )
...
def LIAFilterHomegaSquared2(
    ...
    return a / (
        (1.0 + (2.0 * np.pi * (frequency - demodFreq) * taun) ** 2.0) ** (8.0 / 2.0)
    )
...
def stdLIAPSD(
    ...
    if showwindow:
        ...
        #'tab:brown'
        LIAraw_ax.set_ylabel("Amplitude (Arb. Units)")
        ...
        LIAwindowed_ax.set_ylabel("Amplitude (Arb. Units)")
...
def LIA_FFT(
    TS: np.ndarray = None,
    sampRate: Quantity = None,
    demodFreq: Quantity = 0 * unit.Hz,
    attenuation: Quantity = 0 * unit.dB,
    winInfo: dict = {"name": "rectangle", "param": None},
    zeroPadding: bool = False,
    showWin=False,
    DTRC_filter: str = "off",
    DTRC_filter_TC: float = 1e-6,
    DTRC_filter_order: float = 8,
    verbose: bool = False,
) -> Tuple[Quantity, Quantity, np.ndarray]:
    """
    Return the frequency bin centers and FFT results.
        This function processes data from MFIA lock-in amplifier.
    ...
    ----------
    TS : 2-D array_like
        Time-series data of two output channels X + iY.
    ...
    if sampRate is None or sampRate <= 0:
        raise ValueError("sampRate must be a positive number")

    # Normalize all supported inputs into a 2-D array with shape (Nshots, timeLen).
    ...
    if TS.ndim == 1:
        ...
        if np.iscomplexobj(TS):
            if TS.shape[1] == 1:
                print(
                    f"[{LIA_FFT.__name__}] Warning: 2D TS should have shape (Nshots, timeLen). Your timeLen is 1. "
                )
            # else:
            signal = TS
        else:
            raise ValueError(
                f"[{LIA_FFT.__name__}] 2D TS must be complex. If your data is stored as two real columns, please convert it to complex and keep one shot."
            )
    else:
        raise ValueError("TS must be a 1D or 2D array")

    if signal.ndim != 2:
        # signal = np.atleast_2d(signal)
        raise ValueError("signal must be a 2D array with shape (Nshots, timeLen)")

    timeLen = signal.shape[1]

    if verbose:
        print(f"signal shape: {signal.shape}")

    if isinstance(winInfo, dict):
        window_name = str(winInfo.get("name", "rectangle"))
        param = winInfo.get("param", -10.0)
    elif isinstance(winInfo, str):
        window_name = winInfo
    else:
    ...
    if showWin:
        ...
        win_ax = fig.add_subplot(gs[1, 0], sharex=raw_ax)

        signal_mean = np.mean(signal, axis=0)
        (real_raw_line,) = raw_ax.plot(
            xstamp,
            signal_mean.real,
            label="real",
            color="tab:green",
        ...
        # raw_ax.grid(True)
        # raw_ax2 = raw_ax.twinx()
        # (windowline,) = raw_ax2.plot(
        #     xstamp, window_arr, label="Window", color="tab:blue"
        # )
        # raw_ax2.set_ylabel("Window value", color="tab:blue")
        # raw_ax2.set_ylim(bottom=-0.1 * np.amax(window_arr))

        (real_win_line,) = win_ax.plot(
            xstamp,
            signal_mean.real * window_arr,
            label="real",
            color="tab:green",
            alpha=1,
        )
        (imag_win_line,) = win_ax.plot(
            xstamp,
            signal_mean.imag * window_arr,
            label="imag",
            color="tab:brown",
            alpha=1,
        ...
        # win_ax.grid(True)
        win_ax2 = win_ax.twinx()
        (windowline2,) = win_ax2.plot(
            xstamp,
            window_arr,
            label="Window",
            color="tab:blue",
        ...
        raw_ax.legend(
            handles=[real_raw_line, imag_raw_line],
            bbox_to_anchor=(1.4, 1.0),
            loc="upper left",
        )
        win_ax2.legend(
            handles=[real_win_line, imag_win_line, windowline2],
            bbox_to_anchor=(1.4, 1.0),
            loc="upper left",
        )

        raw_ax.set_title("Before windowing")
        win_ax.set_title("After windowing")

        # plt.tight_layout()
        plt.show()

    attenuation = 0.0 * unit.dB if attenuation is None else attenuation
    demodFreq = 0.0 * unit.Hz if demodFreq is None else demodFreq
    ...
    #
    frequencies = Quantity(np.fft.fftfreq(fft_vals.shape[-1], d=1.0 / sampRate))
    filtercomp = np.ones(frequencies.shape)
    # if DTRC_filter == "on":
    #     filtercomp = LIAFilterHomega(
    #         frequency=frequencies,
    #         taun=DTRC_filter_TC,
    #         order=DTRC_filter_order,
    #     )

    fft_vals = Quantity(
        10.0 ** (attenuation.to_value(unit.dB) / 20.0)
        * (fft_vals / filtercomp)
        / (S2 * sampRate.to_value(unit.Hz)) ** 0.5
    )
    frequencies += demodFreq
    frequencies = Quantity(np.fft.fftshift(frequencies))
    fft_vals = Quantity(np.fft.fftshift(fft_vals, axes=(1,)))
    return frequencies, fft_vals, window_arr
